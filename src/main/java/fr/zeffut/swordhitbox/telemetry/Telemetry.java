package fr.zeffut.swordhitbox.telemetry;
 ...
 * <p>Opt-out:
 * <ul>
 *   <li>{@code -Dswordhitbox.telemetry=false} (system property, wins over everything);</li>
 *   <li>{@code "telemetry": false} in {@code config/swordhitbox.json} (see {@link ModConfig});</li>
 *   <li>auto-OFF when running in a dev environment (system property
 *       {@code swordhitbox.dev=true}), so local runs never pollute the dashboard.</li>
 * </ul>
 *
 ...
 * mod-specific events prefixed with {@code swh_} (emit them via
...
public final class Telemetry {
    ...
    private static final String APP = "swordhitbox";
    /** Prefix for this mod's own (non-standard) events, e.g. {@code swh_feature_used}. */
    public static final String EVENT_PREFIX = "swh";
    ...
    private static final HttpClient HTTP = HttpClient.newBuilder()
            .executor(Executors.newSingleThreadExecutor(r -> {
                Thread t = new Thread(r, "swordhitbox-telemetry");
                t.setDaemon(true);
                return t;
            })).build();
    ...
    private static boolean resolveEnabled() {
        ...
        if (Boolean.getBoolean("swordhitbox.dev")) return false;
        String sys = System.getProperty("swordhitbox.telemetry");
        if (sys != null) return Boolean.parseBoolean(sys);
        return ModConfig.get().telemetry();
    }

    private static String resolveHost() {
        String sys = System.getProperty("swordhitbox.telemetry.host");
